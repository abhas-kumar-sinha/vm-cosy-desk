...
import { Search, Power, LayoutGrid, Maximize2, Minimize2, Command, Cpu } from "lucide-react";
    ...
    <div className="pointer-events-auto fixed inset-x-0 top-0 z-[1000] flex h-8 items-center justify-between px-3 text-xs text-panel-foreground glass-panel">
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          className="rounded-md p-1.5 transition hover:bg-white/10 active:scale-90"
        >
      ...
      {tray && (
        <div className="absolute right-2 top-9 w-72 origin-top-right rounded-xl p-3 text-sm glass-panel animate-in-scale">
          ...
          <div className="grid grid-cols-2 gap-2">
            <TrayTile icon={Search} label="Search apps" onClick={() => { setTray(false); setLauncherOpen(true); }} />
          ...
          >
            <Power className="h-4 w-4" /> Restart Session
          </button>
        </div>
      )}
    </div>
